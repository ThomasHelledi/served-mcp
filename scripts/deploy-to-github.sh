#!/bin/bash
# Deploy MCP docs to GitHub
# Usage: ./scripts/deploy-to-github.sh

set -e

REPO_NAME="served-mcp"
ORG="UnifiedHQ"
DOCS_DIR="$(cd "$(dirname "$0")/.." && pwd)"

echo "=== Deploying MCP Docs to GitHub ==="
...
if ! gh repo view "$ORG/$REPO_NAME" &> /dev/null 2>&1; then
    ...
    gh repo create "$ORG/$REPO_NAME" \
        --public \
        --description "MCP Server for AI assistants to interact with Served platform (Documentation)" \
        --homepage "https://docs.served.dk/mcp"
...
if [ ! -d ".git" ]; then
    git init
    git add -A
    git commit -m "Initial MCP documentation"
fi
